...
/*   Updated: 2022/07/26 20:41:16 by xvoorvaa      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static void	strafing_movement(mlx_t *mlx, char *map[], t_data *s)
{
	...
	{
		if (map[(int)(s->pos_y)][(int)(s->pos_x - s->dir_y * (WALK + 0.01))] == WALL)
			s->pos_x -= s->dir_y * WALK;
		if (map[(int)(s->pos_y + s->dir_x * (WALK + 0.01))][(int)(s->pos_x)] == WALL)
...
static void	walk_movement(mlx_t *mlx, char *map[], t_data *s)
{
	...
	{
		if (map[(int)(s->pos_y)][(int)(s->pos_x - s->dir_x * (WALK + 0.01))] == WALL)
			s->pos_x -= s->dir_x * WALK;
		if (map[(int)(s->pos_y - s->dir_y * (WALK + 0.01))][(int)(s->pos_x)] == WALL)
